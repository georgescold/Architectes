# EP03 — 130 publicités pour les constructeurs, 2 pour les architectes

Épisode 03/20 · PREUVE · architecte DE et architecte d'intérieur · 14-16 min
100 % capture d'écran, aucun accord requis, tout est public.


## LA BIG IDEA

L'espace publicitaire de l'architecture en France est vide, et ce n'est pas une opinion de
vendeur de publicité : c'est consultable publiquement, gratuitement, en dix minutes.

Pendant que la profession considère la publicité comme accessoire, ceux qui prennent ses
projets diffusent en continu depuis des années. Le déséquilibre se compte : 130 publicités
actives sur l'expression « constructeur de maisons », 2 sur « architecte DPLG ».

Et le spectateur repart avec la manipulation exacte pour le vérifier lui-même, puis pour
auditer son propre département.

Ce que la vidéo apporte en réponse :

Voir le vide, puis l'occuper.

Dans la vidéo, je donne la manipulation exacte pour auditer son propre département en dix
minutes : qui diffuse, depuis quand, avec quel message. Et ce que ce relevé montre presque
partout, c'est une place libre.

Occuper cette place ne demande pas un budget de constructeur. Ça demande de diffuser là où
personne ne diffuse — c'est-à-dire au moment où la publicité en ligne coûte le moins cher,
parce que personne n'enchérit en face.


## LE FIL DE LA DÉMONSTRATION — une idée par chapitre

Une idée par chapitre, dans l'ordre. Lues à la suite, elles donnent le raisonnement complet
de la vidéo.

L'idée = ce que le spectateur doit avoir compris à la fin du chapitre.
En clair = la même chose expliquée en détail, pour toi.
À l'écran = ce qu'on montre pendant.

Ch.1 · Ouverture

L'idée : l'écart entre ceux qui font de la publicité et ceux qui n'en font pas se compte. Il
ne se suppose pas.

En clair : 130 publicités actives sur l'expression « constructeur de maisons », 2 sur
« architecte DPLG ». Ce sont des compteurs publics, relevés à une date précise, que
n'importe qui peut revérifier pendant la vidéo.

À l'écran : les deux compteurs côte à côte, date du relevé visible.
Lien : https://www.facebook.com/ads/library/?active_status=active&ad_type=all&country=FR


Ch.2 · D'où je parle

L'idée : cet outil n'est pas une curiosité que tu montres. C'est ce que tu ouvres avant
chaque campagne.

En clair : avant de diffuser pour un cabinet, tu regardes qui diffuse déjà sur sa zone,
depuis quand, et avec quel message. Tu ne découvres pas l'outil devant eux, tu t'en sers
toutes les semaines.

À l'écran : la page d'accueil de la bibliothèque publicitaire, URL lisible.
Lien : https://www.facebook.com/ads/library/?active_status=active&ad_type=all&country=FR


Ch.3 · Les trois limites de l'outil

L'idée : tu dis toi-même ce que l'outil ne sait pas faire, avant que quelqu'un te le
reproche.

En clair : il cherche dans le texte des annonces, pas dans le métier de l'annonceur — donc
« architecte » remonte aussi des écoles et des magazines de décoration. Il ne classe pas par
secteur, donc il est impossible de demander « tous les architectes qui annoncent en
France ». Conséquence directe : jamais « les seuls », jamais « aucun ». Seulement un nombre
de résultats sur une expression exacte, à une date. Ces limites annoncées d'entrée rendent
tout le reste de la vidéo incontestable.

À l'écran : une recherche « architecte » qui remonte des écoles et des magazines.
Lien : https://www.facebook.com/ads/library/?active_status=active&ad_type=all&country=FR


Ch.4 · Ce que vaut un espace publicitaire vide

L'idée : la publicité se vend aux enchères. Quand personne n'enchérit en face, le même
message coûte moins cher.

En clair : ce n'est pas seulement « il y a de la place ». C'est que la place est bon marché.
Deux cabinets qui diffusent le même message, l'un dans un département saturé, l'autre dans
un département vide, ne paient pas le même prix pour la même demande entrante.

À l'écran : le principe de l'enchère, en schéma simple.


Ch.5 · Le piège des grosses audiences

L'idée : un gros nombre de publicités ne veut pas dire un gros budget.

En clair : une même annonce dupliquée sur dix villes apparaît comme dix diffusions alors
qu'il n'y a qu'une seule création derrière. Donc avant de se dire « celui-là est énorme, je
ne peux pas lutter », il faut regarder s'il s'agit de dix messages différents ou du même,
copié dix fois.

À l'écran : une annonce dupliquée par zone, compteur de diffusions en évidence.
Lien : https://www.facebook.com/ads/library/?active_status=active&ad_type=all&country=FR


Ch.6 · La manipulation, en direct

L'idée : tout ce que tu viens de montrer, il peut le refaire ce soir en trois gestes.

En clair : choisir le pays, taper l'expression exacte entre guillemets, régler le statut sur
les annonces actives. Rien d'autre. Tu le fais à l'écran, lentement, curseur visible.

À l'écran : la manipulation filmée, date du jour affichée en bandeau.
Lien : https://www.facebook.com/ads/library/?active_status=active&ad_type=all&country=FR


Ch.7 · Les quatre lectures d'une bibliothèque

L'idée : voir des annonces ne sert à rien. Savoir quoi y lire sert à tout.

En clair : qui diffuse, depuis combien de temps, combien de versions différentes de
l'annonce, et ce que dit le texte. Une annonce diffusée depuis deux ans sans interruption
est une annonce qui rapporte — personne ne paie deux ans pour rien. C'est le vrai cadeau de
la vidéo.

À l'écran : les quatre lectures en liste, puis appliquées à un annonceur réel.
Lien : https://www.facebook.com/ads/library/?active_status=active&ad_type=all&country=FR


Ch.8 · Le cas qui vient de s'arrêter

L'idée : les dates de diffusion racontent ce que les discours cachent.

En clair : un annonceur qui diffusait depuis deux ans et qui s'arrête, ça se voit. Ce que tu
ne sais pas, c'est pourquoi — et tu le dis. Tu montres le fait, pas une interprétation.

À l'écran : la fiche d'un annonceur, date de fin de diffusion visible.
Lien : https://www.facebook.com/ads/library/?active_status=active&ad_type=all&country=FR


Ch.9 · LE BLOC ACQUISITION — deux minutes pleines, jamais expédié

L'idée : l'outil montre un terrain vide. Il ne l'occupe pas. Et un terrain vide ne le reste
pas.

En clair : aujourd'hui, un cabinet qui diffuse dans son département est souvent seul. Le
jour où trois confrères s'y mettent, le prix d'une demande n'est plus le même — c'est le
principe de l'enchère vu au chapitre 4. L'avantage existe maintenant, pas dans trois ans.

À l'écran : la page essort.agency/ressources.
Lien : https://essort.agency/ressources


Ch.10 · Sortie

L'idée : il peut auditer son propre département ce soir, gratuitement, en dix minutes.

En clair : tu ne demandes rien d'autre. Et tu laisses la tension : il va peut-être découvrir
qu'un confrère diffuse déjà.

À l'écran : le champ de recherche, vide, curseur qui clignote.
Lien : https://www.facebook.com/ads/library/?active_status=active&ad_type=all&country=FR


## AVANT DE TOURNER — le mécanisme, expliqué en entier

### Les mots de cet épisode, à ne jamais employer sans les expliquer

Créative. Le visuel d'une publicité : l'image ou la vidéo, avec son texte. Une même campagne
peut tester plusieurs créatives. C'est du vocabulaire d'agence — à expliquer en une phrase la
première fois que tu l'emploies, ou à remplacer par « version de la publicité ».

Enchère. La publicité en ligne ne se vend pas à prix fixe. Plusieurs annonceurs qui veulent
toucher les mêmes personnes enchérissent, et le prix monte avec le nombre de concurrents en
face. C'est le mécanisme qui rend un espace vide moins cher, et il est au cœur du chapitre 4.

Changement de destination. Transformer un local d'un usage vers un autre — un logement en
commerce, par exemple.



### Ce qu'est cette bibliothèque et pourquoi elle existe

C'est un registre public que Meta est obligée de tenir. Toute publicité diffusée sur
Facebook et Instagram y figure, avec sa date de début de diffusion, son texte, son visuel
et son lien de destination. Gratuite, sans compte, accessible à n'importe qui.
https://www.facebook.com/ads/library

Elle existe parce que la transparence publicitaire est une obligation réglementaire,
renforcée en Europe. Un annonceur ne peut pas s'y soustraire : s'il diffuse, il est dedans.

Conséquence pour la vidéo : tout ce que tu montres est vérifiable par le spectateur pendant
qu'il te regarde. C'est ce qui rend cet épisode incontestable — et c'est aussi pour ça que
tu dois être irréprochable sur ce que l'outil permet ou non de conclure.


### Pourquoi un espace vide coûte moins cher

C'est le point que les architectes ne connaissent pas, et c'est de la mécanique d'enchères,
donc ton terrain.

La publicité en ligne fonctionne aux enchères. Quand plusieurs annonceurs veulent toucher
la même audience au même moment, le prix monte. Quand personne d'autre ne la vise, on paie
le prix plancher.

Il y a un second effet, cumulatif celui-là : l'algorithme apprend sur les résultats de
celui qui diffuse. Le premier installé sur une zone accumule cet apprentissage, ce qui
améliore son rendement avec le temps. Un concurrent qui arrive deux ans plus tard paie plus
cher et démarre sans historique.

D'où la formule à retenir : un terrain vide est une fenêtre, et elle se referme d'elle-même
dès que quelques confrères s'y mettent.


### Diffusion, créative, duplication : ce qu'on compte vraiment

Un piège de lecture. Quand la bibliothèque affiche « 890 résultats » pour un annonceur, ça
ne veut pas dire 890 publicités différentes.

Une même créative — un même visuel avec un même texte — peut être dupliquée en dizaines de
diffusions, chacune ciblant une zone ou une audience différente. C'est le cas de Berkail :
890 diffusions pour 12 contenus créatifs seulement. Ça ne dit pas qu'ils produisent
beaucoup, ça dit qu'ils découpent leur territoire.

Savoir lire cette différence, c'est savoir distinguer un annonceur qui teste beaucoup d'un
annonceur qui industrialise une seule idée qui fonctionne.


### Les relevés du 16 septembre 2026

À recapturer le jour du tournage, date visible à l'écran.

Expressions exactes, France, publicités actives : « constructeur de maisons » 130,
« plan 3D gratuit » 97, « architecte DPLG » 2, « architecte restaurant » 0.

L.Decor, Morbihan : 120 diffusions, la plus ancienne ayant tourné du 16 décembre 2024 au
5 novembre 2025 sans interruption. Encore active. Tous les clics partent vers un profil
Instagram ou une messagerie, jamais vers un site.
https://www.facebook.com/ads/library/?active_status=all&ad_type=all&country=FR&view_all_page_id=255425574640770&search_type=page&media_type=all

Berkail, Sud-Est : 890 diffusions pour 12 contenus, diffusion continue du 17 septembre 2025
au 15 septembre 2026, puis arrêt.
https://www.facebook.com/ads/library/?active_status=all&ad_type=all&country=FR&view_all_page_id=101746778948315&search_type=page&media_type=all


### Pourquoi le titre a changé

L'ancien annonçait « les 2 SEULS architectes de France qui font de la pub depuis un an ».
Faux sur trois points : on ne peut pas prouver un « seuls en France », ni L.Decor ni Berkail
ne sont architectes inscrits à l'Ordre — ce sont des cabinets d'architecture d'intérieur —
et Berkail a cessé de diffuser le 15 septembre 2026, pendant la préparation.



## LE PIÈGE — ce que l'outil ne permet pas de dire

La recherche par mot-clé cherche dans le TEXTE des publicités, pas dans le métier de
l'annonceur. « Architecte » remonte des écoles, des fournisseurs de LED et des magazines
de décoration.

Et il n'existe aucun classement par secteur : impossible de demander « tous les architectes
qui annoncent en France ». Donc jamais de « seuls », jamais de « aucun ». Seulement : voilà
le nombre de résultats sur cette expression exacte, à cette date.

Dis ces limites toi-même, au début. C'est ce qui te rend incontestable pour le reste.


## LES OBJECTIONS QUI ARRIVERONT EN COMMENTAIRE

« Tu ne peux pas dire qu'il n'y a que 2 publicités. »
Exact, et je le dis : 2 est un nombre de résultats sur une expression exacte, pas un
recensement.

« Les architectes font de la pub ailleurs, pas sur Facebook. »
Possible, et la bibliothèque ne couvre que Meta. Mais Meta et Instagram concentrent
l'essentiel de la publicité locale grand public en France, et c'est le seul registre public
qui existe.

« Comparer constructeurs et architectes n'a pas de sens. »
C'est le sujet : ils s'adressent au même client. Le client, lui, ne fait pas la distinction.

« Berkail s'est arrêté, donc ça ne marchait pas. »
Hypothèse. L'autre est qu'ils ont rempli leur carnet. Douze mois de diffusion continue et
890 publicités ne se financent pas sans retour. Je donne les deux lectures.


## LE DÉROULÉ


### 1. Ouverture

À faire comprendre : que le déséquilibre se compte au lieu de se supposer.

Exemple de formulation :
« Deux recherches, le même jour, sur le même outil public. Expression exacte,
"constructeur de maisons", France, publicités actives : 130. Expression exacte,
"architecte DPLG" : 2.

Même client, même marché. Et il y a un piège dans cet outil qui fait que presque personne
ne trouve les bons résultats — je vais vous le montrer. »


### 2. D'où je parle

À faire comprendre : que c'est un outil que j'utilise professionnellement, pas une
curiosité.

Exemple de formulation :
« Je fais de l'acquisition pour des cabinets d'architecture. C'est mon métier d'aller
chercher des clients pour les vôtres, et j'ouvre cet outil toutes les semaines. Tout ce
que je vais montrer est public et gratuit : vous pouvez refaire la manipulation pendant
que vous me regardez. »


### 3. Les trois limites de l'outil

À faire comprendre : que je sais ce que l'outil ne sait pas faire. C'est ce qui rend
crédible tout ce qui suit.

Exemple de formulation :
« Trois limites, et je les donne avant tout le reste.

Un : la recherche cherche dans le texte des publicités, pas dans le métier de l'annonceur.
Tapez "architecte" et vous aurez des écoles, des vendeurs de LED et des magazines de déco.

Deux : il n'y a aucun classement par secteur. Je ne peux donc jamais dire "il n'y a que X
architectes qui font de la pub en France". Je peux seulement dire : voilà le nombre de
résultats sur cette expression, aujourd'hui.

Trois : les compteurs bougent tous les jours. Celui que je vous montre date du jour du
tournage. »


### 4. Ce que vaut un espace publicitaire vide

À faire comprendre : la mécanique des enchères, qui est son domaine.

Exemple de formulation :
« Pourquoi ça m'intéresse qu'un espace soit vide ? Parce que la publicité fonctionne aux
enchères. Quand personne n'enchérit sur une audience, elle coûte moins cher — vous payez
le prix plancher. Et le premier qui s'installe sur une zone prend une avance qui se
cumule, parce que l'algorithme apprend sur ses résultats à lui.

Mais ce n'est pas éternel. Le jour où trois confrères de votre département s'y mettent,
les enchères montent pour tout le monde, y compris pour vous. »


### 5. Le piège des grosses audiences

À faire comprendre : qu'une audience ne dit rien de la dépense, et qu'il faut vérifier
avant de copier quelqu'un.

Exemple de formulation :
« On me dit souvent : "untel a 50 000 abonnés, il doit bien faire de la pub". J'ai vérifié.
Un compte parisien très suivi, 52 600 abonnés en architecture d'intérieur : douze
publicités en tout, par salves de quelques heures.

Grosse audience ne veut pas dire dépense publicitaire. Avant de prendre quelqu'un pour
modèle, ouvrez sa bibliothèque. »

Nom masqué à l'écran.


### 6. La manipulation, en direct

À faire comprendre : qu'il peut tout refaire lui-même ce soir, en trois gestes.

Exemple de formulation :
« Premier geste : l'expression exacte, entre guillemets. Sans ça vous comparez n'importe
quoi.

Deuxième geste : le mode annonceur. Vous tapez un nom dans le champ de recherche et la
liste déroulante vous donne la page ET son compte Instagram lié, avec le nombre d'abonnés.
C'est le seul endroit d'internet où vous avez les deux d'un coup.

Troisième geste, et c'est celui que personne ne connaît : par défaut la bibliothèque ne
montre que les publicités actives. Pour voir l'historique complet, il faut changer l'URL.
Sans ça, vous croyez qu'un concurrent débute alors qu'il diffuse depuis deux ans. »

URL à montrer en grand :
https://www.facebook.com/ads/library/?active_status=all&ad_type=all&country=FR&view_all_page_id=IDENTIFIANT&search_type=page&media_type=all


### 7. Les quatre lectures d'une bibliothèque

À faire comprendre : comment interpréter ce qu'il voit. C'est le cadeau de la vidéo.

Exemple de formulation :
« Une fois la page ouverte, je regarde quatre choses.

Depuis quand il diffuse sans trou. Une publicité gardée six mois n'a pas été gardée par
sentimentalisme : elle rapporte.

Combien de contenus différents. Beaucoup de diffusions pour peu de contenus, c'est de la
duplication par zone, pas de la production.

Où va le clic. Profil Instagram, messagerie ou formulaire : ce choix raconte toute la
stratégie.

Et s'il rediffuse d'anciens visuels. C'est le signal le plus fiable qu'ils fonctionnaient.

Exemple en direct : ce cabinet du Morbihan, 120 diffusions, une publicité qui a tourné
onze mois d'affilée, et tous les clics vers un profil Instagram. Pas de site, pas de
formulaire. C'est une stratégie assumée, pas un hasard. »


### 8. Le cas qui vient de s'arrêter

À faire comprendre : que les dates disent ce que les discours cachent, et qu'on reste
honnête sur ce qu'on ne sait pas.

Exemple de formulation :
« Un autre cas, et il est intéressant parce qu'il vient de bouger. Un atelier du Sud-Est
a diffusé sans interruption pendant douze mois — 890 diffusions pour douze contenus. Et il
s'est arrêté le 15 septembre.

Deux lectures possibles, je vous donne les deux : soit ça ne rapportait plus, soit le
carnet est plein. Douze mois de diffusion continue ne se financent pas sans retour. Je ne
sais pas laquelle est la bonne, et c'est exactement pour ça qu'on regarde les dates plutôt
que d'écouter les avis. »


### 9. LE BLOC ACQUISITION — deux minutes pleines, jamais expédié

À faire comprendre : que l'outil montre un terrain vide mais ne l'occupe pas, et que la
fenêtre se refermera.

Exemple de formulation :
« Maintenant vous savez lire ce que font les autres. Et ça ne vous apporte pas un seul
client. Cet outil sert à regarder, pas à occuper.

Ce que je vois, moi, en ouvrant ça toutes les semaines : un terrain où presque personne
n'enchérit, dans un métier où le client cherche activement. C'est une situation qui
n'arrive pas souvent, et qui ne dure jamais. Les enchères montent avec le nombre
d'annonceurs — ceux qui arrivent les premiers payent moins cher, et plus longtemps.

La publicité en ligne est aujourd'hui, sur ce métier et dans la plupart des départements,
le levier le moins cher pour être devant quelqu'un qui a un projet maintenant. Pas dans
six mois quand le référencement aura pris. Maintenant. »

Puis, au nous :
« Chez Essort, c'est la première chose qu'on fait quand un cabinet nous appelle : on ouvre
la bibliothèque sur son département avant même de parler de budget. En vingt minutes on
sait qui dépense autour de lui, depuis quand et sur quoi. Ça évite de proposer un plan qui
ignore ce que le voisin fait déjà depuis deux ans. »

Puis le document, enchaîné sans respirer :
« Tous les leviers d'acquisition, ce que chacun coûte, dans quel ordre les monter :
document de 57 pages, gratuit, lien en description. Pas de carte bancaire, pas d'appel
commercial. »
https://essort.agency/ressources


### 10. Sortie

À faire comprendre : qu'il peut agir ce soir, et rouvrir une tension.

Exemple de formulation :
« Ce soir, dix minutes : ouvrez la bibliothèque et tapez le nom de votre département avec
"rénovation" ou "constructeur". Vous saurez qui vous fait concurrence sur l'attention.

Et si vous vous dites que de toute façon, vous n'avez pas le droit de faire de la
publicité : c'est faux depuis 1992. Je vous montre le texte dans la vidéo suivante. »


## TITRE ET MINIATURE

Principal : 130 publicités pour les constructeurs, 2 pour les architectes
Variante : Votre département n'a probablement aucune publicité d'architecte. Vérifiez en 10 minutes.
Variante : L'outil gratuit qui vous montre qui prend vos projets, et depuis quand

Miniature : « 130 / 2 » en énorme, dessous en petit « même client ». Visage à gauche,
les deux comptages côte à côte à droite.


## DESCRIPTION YOUTUBE

```
La bibliothèque publicitaire de Meta est publique et gratuite. En dix minutes elle dit
qui dépense, depuis quand, et où il envoie les gens. Voici la manipulation exacte, et ce
qu'elle révèle sur le marché de l'architecture en France.

Relevés du jour du tournage. Les compteurs bougent : refaites la recherche.

Je fais de l'acquisition pour des cabinets d'architecture. J'ouvre cet outil toutes les
semaines.

Les leviers d'acquisition, ce que chacun coûte et dans quel ordre les monter — 57 pages,
gratuit : https://essort.agency/ressources

SOURCES
Bibliothèque publicitaire Meta : https://www.facebook.com/ads/library

Instagram : https://instagram.com/essort.architectes
```

Mots-clés : bibliothèque publicitaire Meta · espionner publicités concurrents · Facebook
ads library France · publicité architecte · audit concurrence


## GARDE-FOUS

Ne jamais dire « les seuls en France » : l'outil ne recense pas un secteur.
Ne jamais appeler « architecte » un annonceur non inscrit à l'Ordre.
Recapturer tous les comptages le jour du tournage, date visible.
Masquer le nom du compte à forte audience.
Ne pas présenter l'arrêt de Berkail comme un échec : donner les deux lectures.
Constructeurs, cuisinistes et plateformes ne sont pas des confrères, on peut les nommer.
Les cabinets d'architecture d'intérieur sont des pairs du spectateur : on les cite sans
jamais les juger.
