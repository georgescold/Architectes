...
3. **Le lien est la recherche exacte, jamais la page d'accueil de l'outil.** Un lien vers
   `facebook.com/ads/library` ne sert à rien : au tournage il faut arriver directement sur
   le compteur qu'on montre. Donc l'URL porte la requête, le mode de recherche
   (`keyword_exact_phrase` ou `keyword_unordered`), le pays et le statut. Même logique pour
   un texte de loi : on lie l'article, pas le code entier.
4. **Le lien est sous la ligne « À l'écran », jamais ailleurs.** Dès qu'un chapitre montre
...
5. **Rien ne se désigne sans se nommer.** Jamais « les trois autres » ou « ce mécanisme » :
   on écrit lesquels, à chaque fois, même si c'est répété.
6. **Toute capture qui contient un chiffre porte la date du relevé à l'image**, et aucun
