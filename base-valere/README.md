# Base Valère — la méthode

Ce dossier est la base méthodologique utilisée par l'agence. Le travail de l'agence
elle-même est dans [`../agence/`](../agence/README.md).

> **Mises à jour du template.** Ce dossier vient du dépôt `georgescold/Process-Valere`
> (remote `template`), où il est rangé à la racine. `git pull template main` suit les
> renommages dans la plupart des cas ; un fichier ajouté par le template peut toutefois
> arriver à la racine : le déplacer alors dans `base-valere/`.
